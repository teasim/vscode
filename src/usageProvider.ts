...
import { env, Location, languages, Range, Uri, window, workspace } from "vscode";
...
export class UsageReferenceProvider implements ReferenceProvider {
  ...
  constructor(public loader: ContextLoader) {}

  public async provideReferences(document: TextDocument, position: Position, _context: ReferenceContext, _token: CancellationToken): Promise<Location[] | undefined> {
    const ctx = await this.loader.resolveClosestContext(document.getText(), document.uri.fsPath);
    if (!ctx || !ctx.filter(document.getText(), document.uri.fsPath)) return undefined;
    ...
    if (!this.warned) {
      window.showInformationMessage(`[UnoCSS] Find all references is an experimental feature`, "Dismiss", "Report Issue").then((r) => {
        if (r !== "Report Issue") return;
    ...
    const files = await workspace.findFiles(`**/*.{vue,html,js,ts,jsx,tsx,svelte,astro,elm,php,phtml,mdx,md,marko}`, "**/{node_modules,.git,dist,.output,.cache,cache}/**", 1000);

    const result = (
      await Promise.all(
        files.map(async (f) => {
          const doc = await workspace.openTextDocument(f);
          if (!ctx.filter(doc.getText(), f.fsPath)) return undefined;
    ...
    if (!target) {
      return positions.filter((i) => i[2] === name).map((i) => new Location(document.uri, new Range(document.positionAt(i[0]), document.positionAt(i[1]))));
    }
    ...
    const names = new Set([
      ...
      ...[...cacheMap.entries()]
        .filter(([_, utils]) => {
          ...
          if (utils.length !== target.length) return false;
    ...
    return Object.values(result).flatMap(({ doc, positions }) => {
      return positions.filter((i) => names.has(i[2])).map((i) => new Location(doc.uri, new Range(doc.positionAt(i[0]), doc.positionAt(i[1]))));
    });
  }
}
...
export async function registerUsageProvider(loader: ContextLoader) {
  ...
  loader.ext.subscriptions.push(languages.registerReferenceProvider(await getLanguageIds(), provider));
}
