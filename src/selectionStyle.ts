...
export async function registerSelectionStyle(loader: ContextLoader) {
  ...
  async function selectionStyle(editor: TextEditorSelectionChangeEvent) {
    try {
      if (!config.selectionStyle) return reset();
      ...
      const range = new Range(new Position(selection.start.line, selection.start.character), new Position(selection.end.line, selection.end.character));
      let code = editor.textEditor.document.getText(range).trim();
      if (!code.startsWith("<")) code = `<div ${code}`;
      ...
      if (result.length <= 1) return reset();

      const uniqMap = new Map<string, string>();
      for (const [start, end, className] of result) uniqMap.set(`${start}-${end}`, className);

      const classNamePlaceholder = "___";
      const sheetMap = new TwoKeyMap<string | undefined, string, string>();
      await Promise.all(
        Array.from(uniqMap.values()).map(async (name) => {
          if (!ctx) return;
