...
export function createContext<Config extends UserConfig<any> = UserConfig<any>>(configOrPath?: Config | string, defaults: UserConfigDefaults = {}, extraConfigSources: LoadConfigSource[] = [], resolveConfigResult: (config: LoadConfigResult<Config>) => void = () => {}): UnocssPluginContext<Config> {
  ...
  function invalidate() {
    invalidations.forEach((cb) => void cb());
  }

  function dispatchReload() {
    reloadListeners.forEach((cb) => void cb());
  }
