...
export function getPlainClassMatchedPositionsForPug(codeSplit: string, matchedPlain: Set<string>, start: number) {
  ...
  matchedPlain.forEach((plainClassName) => {
    ...
    if (plainClassName.includes(":")) {
      if (plainClassName === codeSplit) result.push([start, start + plainClassName.length, plainClassName]);
    } else {
...
export function getMatchedPositions(code: string, matched: string[], extraAnnotations: HighlightAnnotation[] = [], options: GetMatchedPositionsOptions = {}) {
  ...
  if (options.includeRegex) {
    for (const regex of options.includeRegex) {
      for (const match of code.matchAll(regex)) includeRanges.push([match.index!, match.index! + match[0].length]);
    }
  } else {
...
const ignoreTransformers = ["@unocss/transformer-directives", "@unocss/transformer-compile-class"];

export async function getMatchedPositionsFromCode(uno: UnoGenerator, code: string, id = "", options: GetMatchedPositionsOptions = {}) {
