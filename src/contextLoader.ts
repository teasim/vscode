...
export class ContextLoader {
  ...
  private async _reload() {
    ...
    for (const dir of dirs) await this.loadContextInDirectory(dir);
    if (!dirs.length) await this.loadContextInDirectory(this.cwd);
  }
  ...
  async unload(configDir: string) {
    ...
    for (const [path, ctx] of this.fileContextCache) {
      if (ctx === context) this.fileContextCache.delete(path);
    }
  ...
  async configExists(dir: string) {
    if (!this.configExistsCache.has(dir)) {
      ...
      this.configExistsCache.set(
        dir,
        files.some((f) => frameworkConfigRE.test(f)),
      );
    }
  ...
  async loadContextInDirectory(dir: string) {
    ...
    const load = async () => {
      ...
      context.onReload(() => {
        for (const [path, ctx] of this.fileContextCache) {
          if (ctx === context || !ctx) this.fileContextCache.delete(path);
        }
        this.configExistsCache.clear();
      ...
      for (const [path, ctx] of this.fileContextCache) {
        if (!ctx) this.fileContextCache.delete(path);
      }
    ...
    if (context) this.registerEditorSupport();

    return context;
  }

  async resolveClosestContext(code: string, file: string) {
    if (!this.contextsMap.size) return undefined;

    if (excludeFileRE.test(file)) return undefined;

    if (this.fileContextCache.has(file)) return this.fileContextCache.get(file);

    const entries = Array.from(this.contextsMap.entries()).sort((a, b) => b[0].length - a[0].length);
    for (const [configDir, context] of entries) {
      if (!context) continue;

      if (!isSubdir(configDir, file)) continue;

      if (!context.filter(code, file) && !isCssId(file)) continue;
