...
async function rootRegister(ext: ExtensionContext, root: string[], status: StatusBarItem) {
  ...
  const configNames = ["uno.config.js", "uno.config.ts", "unocss.config.js", "unocss.config.ts"];

  const registerUnocss = async (url = window.activeTextEditor?.document.uri.fsPath) => {
    if (!url) return;

    if (cacheFileLookUp.has(url)) return;
    ...
    if ([...rootCache].some((root) => url.startsWith(root))) return;
