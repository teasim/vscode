...
export async function registerAnnotations(loader: ContextLoader) {
  ...
  disposals.push(
    config.watchChanged(["underline", "colorPreview", "remToPxPreview", "remToPxRatio"], () => {
      updateAnnotation();
    }),
  );
  ...
  async function updateAnnotation(editor = window.activeTextEditor) {
    try {
      ...
      if (!doc) return reset(editor);

      const id = doc.uri.fsPath;
      if (!loader.isTarget(id)) return reset(editor);

      const code = doc.getText();
      if (!code) return reset(editor);

      const ctx = await loader.resolveClosestContext(code, id);
      if (!ctx) return reset(editor);

      const isTarget =
        ctx.filter(code, id) || // normal unocss filter
      ...
      if (!isTarget) return reset(editor);
      ...
      const remToPxRatio = docConfig.remToPxPreview ? docConfig.remToPxRatio : -1;
  ...
  disposals.push(
    workspace.onDidChangeTextDocument((e) => {
      if (e.document === window.activeTextEditor?.document) throttledUpdateAnnotation();
    }),
  );
